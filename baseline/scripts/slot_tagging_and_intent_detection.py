...
parser.add_argument('--sc_type', default='multi_cls_BCE', help='single_cls_CE | multi_cls_BCE')
...
def decode(data_feats, data_tags, data_class, output_path):
    ...
    TP3, FP3, FN3, TN3 = 0.0, 0.0, 0.0, 0.0
    ...
    if TP3 == 0:
        ...
        ovp, ovr, ovf = 100*TP3/(TP3+FP3), 100*TP3/(TP3+FN3), 100*2*TP3/(2*TP3+FN3+FP3)
    mean_losses = np.mean(losses, axis=0)
    return mean_losses, f, cf ,ovf
...
if not opt.testing:
    ...
    for i in range(opt.max_epoch):
        ...
        # Evaluation
        start_time = time.time()
        loss_val,f_val, cf_val, ovf_val = decode(valid_feats['data'], valid_tags['data'], valid_class['data'], os.path.join(exp_path, 'valid.iter'+str(i)))
        logger.info('Validation:\tF1 : %.2f\tcls-F1 : %.2f\toverall-F1 : %.2f ' % (f_val, cf_val, ovf_val))
        start_time = time.time()
        loss_te, f_te, cf_te, ovf_te = decode(test_feats['data'], test_tags['data'], test_class['data'], os.path.join(exp_path, 'test.iter'+str(i)))
        logger.info('Evaluation:\tF1 : %.2f\tcls-F1 : %.2f\toverall-F1 : %.2f ' % (f_te, cf_te,ovf_te ))
